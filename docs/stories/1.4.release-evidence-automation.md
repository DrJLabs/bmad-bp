...
- Risk profile: docs/qa/assessments/release-governance.story-1.4-risk-20250926.md (Score 62/100 – High). Focus mitigations on CLI run-ID validation, credential redaction, and artifact pruning before gate sign-off.
- Test design: docs/qa/assessments/release-governance.story-1.4-test-design-20250926.md (P0 scenarios defined). Execution must prove non-skipped release run, automation persistence, and negative-path handling.
...
- `tools/release/capture-release-evidence.js`: Helpers validated; added skip flag logging and stricter error handling. Recommend running command post-merge to exercise live gh output.
...
- `npm run reviewer:scan` (artifacts in `artifacts/reviewer/20250926T023210Z`, Semgrep=0, JSCPD duplicates=legacy docs; follow-up issue logged at `docs/bmad/issues/release-governance-duplicate-content.md`).
- `npm run format:check`

#### Coverage & NFR References

- Trace matrix: docs/qa/assessments/release-governance.story-1.4-trace-20250926.md (AC1–AC2 partial, AC3 uncovered).
- NFR assessment: docs/qa/assessments/release-governance.story-1.4-nfr-20250926.md (Reliability = CONCERNS until live release run captured).

#### Follow-Ups / Recommendations

- Execute `npm run release:evidence -- --run-id <main-run>` after next release-worthy commit and append outputs to Story 1 change log (satisfies AC3 + closes reliability concern).
...
Gate: CONCERNS → docs/qa/gates/release-governance.story-1.4-automate-release-evidence-capture-post-change-validation.yml
